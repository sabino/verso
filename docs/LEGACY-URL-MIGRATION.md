...
Optional bounded scalar metadata fields are `sourceRepository`, `sourceBranch`, `worldNodeSourceCommit`, `worldEndpoint`, `voiceEndpoint`, `worldNodeEndpoint`, `worldUrl`, `voiceUrl`, `version`, `sourceDate`, `frontendUrl`, `legacyUrl` and `codec`; they do not affect URL authority or imports. Missing and null optional metadata are accepted. This is a same-origin publishing trust boundary, not a signed-release or hostile-origin integrity system. The entry and declared styles use browser subresource integrity; the worker verifies every cached project asset against the manifest. The four legacy shell files come from the fixed same-origin legacy URLs and remain bounded, separately cached shell responses.

The manifest also describes the canonical `index.html`, but the legacy bridge deliberately does not download, hash or cache that exact document. Existing edge HTML transformations can change its delivered bytes. The bridge never uses it: legacy navigation serves the separately cached `/games/verso/index.html` shell, and canonical `/verso/` navigation belongs to the canonical page and worker. This exception does not apply to any module, lazy chunk, stylesheet, media, image, font or other declared asset. Their integrity checks remain mandatory; a transformed executable or other used asset must fail staging and preserve the prior complete release.
...
2. Installation downloads the fixed release, checks every declared asset except the unused canonical `index.html` for exact byte length and SHA-256, and caches the four legacy shell files. Only a complete cache of the bridge's required assets receives a release marker and becomes the current pointer. Failed fetches, integrity checks or cache writes leave the previous release intact. An origin-wide Web Lock serializes staging and cleanup across active and installing workers; without Web Locks, installation/refresh refuses all cache mutation. Each network response has a 30-second deadline covering its body, including the loader's initial manifest.
...
`tests/legacy-pages-bridge.test.ts` executes the actual loader and worker logic with an in-memory Cache API. It covers strict manifests, body bounds, invitation preservation, old PWA identity, failed and successful cache upgrades, two-release retention, request isolation, transformed unused canonical HTML and mandatory integrity checks for used assets. This is not a native browser upgrade certification. Before publication, use a held old worker/PWA and real saved life to verify online upgrade, closed-and-reopened activation, offline reload and movement, invitations, and canonical installation separately. Keep the old-tab limitation in the release handoff.
